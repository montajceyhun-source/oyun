...
// Qeyd: avtomatik vaxt limiti (sayğac) ləğv edilib — hər lotu aparıcı özü
// "Satıldı!" / "Alıcısız qaldı" düymələri ilə əl ilə bitirir.
...
function doPost(e) {
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'deleteGame': return jsonOut(deleteGame(body));
...
// İştirakçı ad girmir — hər kəsə avtomatik unikal random nömrə verilir (2 rəqəmli: 10-99)
function generateParticipantName(game) {
  let name;
  do {
    name = 'İştirakçı-' + Math.floor(10 + Math.random() * 90);
...
function startLot(body) {
  ...
  game.bidHistory = [];
  game.status = 'active';
  game.lotEndsAt = null; // vaxt limiti yoxdur — aparıcı əl ilə bitirir
  saveGame(game);
  return { game: publicGame(game) };
}
...
// Otağı tamamilə silir (aparıcı otağı ləğv etmək istəyəndə istifadə olunur)
function deleteGame(body) {
  const game = loadGame(body.code);
  if (!game) return { error: 'Oyun tapılmadı' };
  if (!checkAuth(game, body)) return { error: 'Yanlış Admin PIN' };
  PropertiesService.getScriptProperties().deleteProperty('game_' + body.code);
  return { deleted: true };
}
